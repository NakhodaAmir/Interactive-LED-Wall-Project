...
public class ShootHomingProjectile extends Decision {
    ...
    public ShootHomingProjectile(Agent agent, String name, double timer) {
        super(agent, name);
    ...
    public void doAction(double time, Level level) {
        ...
        if (timer.tick(time) > 0) {
            ...
            getAgent().fireProjectile(new EnemyHomingProjectile(getAgent().getLocation().getX(), getAgent().getLocation().getY()), 10, level);
        }
    }
}
