...
public class Scatter extends Decision {
    ...
    public Scatter(Ghost ghost, PacmanGame pacmanGame, String name) {
        super(ghost, name);
        this.ghost = ghost;
    ...
    public void doAction(double time, Level level) {
        if (PacmanUtils.canAct(ghost, time, lastLocation)) {
            Vector2D bestDir = PacmanUtils.getBestDirection(PacmanUtils.getValidDirs(pacmanGame.getCurrentLevel(), ghost), ghost.getLocation(), PacmanUtils.getScatterTarget(pacmanGame.getCurrentLevel(), ghost));
            if (bestDir != null) {
